...
### T-2.4 · Grey-box test world — `done`
...
- **Landed** `data/testWorld.ts` (typed layout: kerbs 0.15/0.35/0.5, ramps 20/45/50°, a five-step stair run, 0.9 m and 0.5 m gaps bracketing the 0.7 m capsule, a wall corner, a 3×3 box grid, a 1.8 m human proxy, and `TEST_SPAWN`) and `world/TestWorld.ts` (builds it, registers every collider under the single owner `test_world`). Four draw calls against the budget of ten, via one `InstancedMesh` of a unit cube for the blocks and one of a unit wedge for the ramps — pitch is a non-uniform scale, so every ramp shares one geometry. `rendering/validationScene.ts` lost its ground plane and test box as required; its lighting and camera aim stay until T-3.10 / T-2.6. Tests in `tests/unit/test-world.test.ts` (6 cases) cover collider-per-piece, bulk teardown, the draw-call budget and the ramp pitch reading back through `groundAt`.
- **Also landed here, beyond the documented file list** `physics/PhysicsSystem.ts`: nothing had ever *constructed* a `CollisionWorld`, and no T-2 task's file list includes an owner for it. `SYSTEM_IDS` has reserved the `'physics'` id since T-1.3 and `ARCHITECTURE.md` §6 gives this module the collision world, so the owner lives there; consumers use `ctx.get<PhysicsSystem>('physics').world` (via the `physicsOf` helper), which is §1's published-interface route. The `GameContext.physics` service slot stays unused for the reason recorded in T-2.1.
- **Two consequences worth knowing** **(a)** `__DEV__` is false in the e2e build as well as production (`vite.config.ts` defines it as `command === 'serve'`), so a strictly `__DEV__`-gated playground would have emptied the scene the T-1.9 smoke test measures. The gate is therefore `(__DEV__ || __E2E__) && ?testworld=1`, and the smoke spec navigates with that flag. Production still drops the module graph entirely — verified by grepping a production build for `test_world`. **(b)** Because the playground is dev/e2e content and T-1.5a's geometry is gone, **the deployed Pages build now renders a lit but empty scene** until Phase 3 lands the real world. That follows the documents rather than contradicting them, but it does mean the live URL no longer shows the M1 box — see the note under T-2.9 about whether M2 wants a deployable scene.
- **Risk / Complexity** Low / S

### T-2.5 · Player controller and movement state machine — `done`
...
- **Landed** `data/balance.ts` (every §3 constant, plus the derived `MAX_WALKABLE_SLOPE_COS` — T-2.7 tunes this file and nothing else), `player/states.ts` (the six states with `enter`/`update`/`exit`, each returning the next state id from an explicit `StateInput` snapshot, so nothing anywhere asks `isGrounded && !isJumping`), `player/PlayerSystem.ts` (§5's nine steps in order, `PlayerView` published as one never-reallocated object, `alpha` interpolation in `lateUpdate`, `debugCapsule()` for T-2.3), the `player` branch of `GameState` that `core/state.ts` had been reserving, and `CollisionWorld.stepUpTo` (the terrain function and narrowphase are both that class's). 22 cases in `tests/unit/player-states.test.ts`: every documented transition, effects on enter/exit, the two speeds, deceleration, jump apex against the constants, coyote time honoured and expired, jump buffer honoured and expired, the 0.35 m climb and the 0.5 m refusal, view/state publication without reallocation, alpha interpolation, determinism, and blocked input reading as neutral.
- **Defect found by its own test** step-up lifted the capsule to the step's height but left it behind the step's face, so it was simply airborne and gravity took it straight back down — it hovered at the kerb instead of climbing it. The lift is now followed by a second horizontal sweep from the raised position, which is what carries the player onto the step.
